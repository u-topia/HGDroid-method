...
class Average(Layer):
    ...
    def call(self, x):
        ...
        if mask is not None:
            ...
            x = x * mask
        '''
        return K.sum(x, axis=-2) / self.length
...
class AttLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        self.W2 = self.add_weight(shape = (1,),
                                ...
                                initializer = 'glorot_uniform',
                                trainable = True)
        self.W3 = self.add_weight(shape = (1,),
                                name = 'weights',
                                initializer = 'glorot_uniform',
                                trainable = True)
        self.W4 = self.add_weight(shape = (1,),
                                name = 'weights',
                                initializer = 'glorot_uniform',
                                trainable = True)
        self.W5 = self.add_weight(shape = (1,),
                                name = 'weights',
                                initializer = 'glorot_uniform',
                                trainable = True)
        self.W6 = self.add_weight(shape = (1,),
                                name = 'weights',
                                initializer = 'glorot_uniform',
                                trainable = True)
        self.W7 = self.add_weight(shape = (1,),
                                name = 'weights',
                                initializer = 'glorot_uniform',
                                trainable = True)
        self.built = True
    ...
    def call(self, inputs):
        weights = K.dot(inputs, self.W)
        return K.sum(weights, axis=-1)
...
class Change_dim(Layer):

    def __init__(self, output_dim, **kwargs):
        # self.input_feature = input_feature
        self.output_dim = output_dim
        super(Change_dim, self).__init__(**kwargs)
    ...
    def call(self, inputs):
        ...
        for i in range(len(inputs)):
            out = []
            for j in range(len(inputs) / self.output_dim):
                out.append(inputs[k])
                k += 1
            outputs.append(out)
        return outputs
    ...
    def get_output_shape_for(self, input_shape):
        return (input_shape[0:-2], self.output_dim, input_shape[-1:] / self.output_dim)
